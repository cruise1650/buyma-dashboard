...
export async function POST(request: Request) {
  ...
  let body: { tab?: string; input?: Record<string, string>; prompt?: string };
  try {
  ...
  let prompt: string;
  const isDirectPrompt = typeof body.prompt === "string";
  ...
  if (isDirectPrompt) {
    ...
    prompt = buildPrompt(tab as SourcingTab, body.input ?? {});
  }
  ...
  try {
    ...
    return isDirectPrompt ? NextResponse.json({ text }) : NextResponse.json({ result: text });
  } catch (err) {
